...
PLATFORM=armv7
DOCKER_COMPOSE_VERSION=$(get_latest_github_release --repo "docker/compose")
DOCKER_COMPOSE_VERSION="${DOCKER_COMPOSE_VERSION:1}"
DOCKER_COMPOSE_VERSION_CURRENT=$(sed -n "s/^DOCKER_COMPOSE=\(.*\).*$/\1/p" ${CONCORDIA_DIR}/versions.env)
...
sed -i "s/DOCKER_COMPOSE=.*/DOCKER_COMPOSE=$DOCKER_COMPOSE_VERSION/g" ${CONCORDIA_DIR}/versions.env
