...
PIAWARE_VERSION=$(get_latest_github_release "flightaware/piaware")
PIAWARE_VERSION="${PIAWARE_VERSION:1}"
PIAWARE_VERSION_CURRENT=$(sed -n "s/^PIAWARE=\(.*\).*$/\1/p" versions.env)
...
if [ "$PIAWARE_VERSION" == "$PIAWARE_VERSION_CURRENT" ]; then
    ...
    sudo apt update
    sudo wget https://flightaware.com/adsb/piaware/files/packages/pool/piaware/p/piaware-support/piaware-repository_$PIAWARE_VERSION_all.deb
